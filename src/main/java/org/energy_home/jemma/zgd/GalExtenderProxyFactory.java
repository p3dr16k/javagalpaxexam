...
public class GalExtenderProxyFactory {
    ...
    public GalController getGal()
    {
        return gal;
    }
    
        
    public void writeWrapper(byte[] buff)
    {
        try
        {
            gal.getDataLayer().getIKeyInstance().write(new ByteArrayObject(buff, buff.length));
        } 
        catch(Exception ex)
        {
            Logger.getLogger(GalExtenderProxyFactory.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    public byte[] readWrapper()
    {
        return ((SerialPortConnectorRxTx)gal.getDataLayer().getIKeyInstance()).getLastPacketReaded();
    }
